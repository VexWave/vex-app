...
- **The app can't delete its own install**, so the uninstall hands a script to a helper and quits — Windows holds an executing image open, and the tree being removed is the one every VexWave process runs out of. Quitting is part of the removal, not what follows it. The same exclusions that guard the yt-dlp updater guard this, for the same reason.
- **That helper waits for the app's pid to go before it deletes anything.** Deleting around a live app takes whatever happens to be unlocked and leaves the rest, which is a half-removed install rather than a failed one.
- **Then it stops whatever is still running out of the tree.** Quitting force-exits, which orphans the CEF helpers rather than ending them, and one of those keeps `CEF\BrowserMetrics\*.pma` mapped — a mapped file can't be deleted at all, so there is nothing to wait out. It gives up on the rest only while the app's executable is still there: past that the install is unusable, and shortcuts into it point nowhere.
- **It is started through WMI, not from here**, so that whatever reaps the app's children on the way down can't reap it too — and it reports in through a file before this side agrees to quit, since quitting into a helper that never started would close the app on a removal that never happens. It logs each branch beside itself in the temp dir: by the time any of it runs there is no app left to report through.
