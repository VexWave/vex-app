...
- **Volume crosses a loudness taper at `AudioPlayer`**: everything outside it — the snapshot, the slider, what is persisted — is a position on the control, while the element and `Drive`, which reads the element, are in amplitude.
