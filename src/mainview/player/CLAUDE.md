...
- **Speed and volume are both element properties, so they sit upstream of the capture.** Speed costs nothing for it: the equalizer, the analyser and the glow follow a speed change without being told. Volume is why `Drive` exists — it is already spent by the time the graph sees the track, so the stage folds it into the slope of its own saturation curve and re-applies it after, and watches `volumechange` itself to keep up.
- **Nothing in `Drive` follows the signal.** Its curve and its makeup are functions of the slider and the volume alone — the makeup an integral over a model of program material, not a measurement — so the level it hands on moves only when a control does. Measuring what saturation actually costs rides the track's own dynamics back at it.
