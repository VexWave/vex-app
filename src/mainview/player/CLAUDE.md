...
- **`Effects` is a second store of the same kind** (`PlayerController.effects`, read through `hooks/useEffects`), holding playback speed and the reverb. **Speed is an element property, so it sits upstream of the capture** — the equalizer, the analyser and the glow all follow a speed change without being told. **The reverb's dry path is a bare connection**, so the track reaches the mix at full level wherever the slider sits; what the slider moves is the room, morphing between the two responses `roomImpulse.ts` synthesises and raising the wet send, with a trim on the summing node holding the whole bus at one level. It is **session-only** — every launch opens on the defaults.
