...
- **`Effects` is a second store of the same kind** (`PlayerController.effects`, read through `hooks/useEffects`), holding playback speed and the reverb. Only the reverb is nodes: **speed is an element property, so it sits upstream of the capture** — the equalizer, the analyser and the glow all follow a speed change without being told. Unlike the equalizer it is **session-only** — nothing here is persisted, and every launch opens on the untouched defaults.
