...
- A settings panel is a `Group` from `SettingsControls` (`EqualizerPanel`, `DiscordPanel`, `UninstallPanel`), holding `SettingRow`s where it has more to say than the switch in its header. A panel need not set anything: `UninstallPanel` holds one action, goes last, and draws nothing where that action isn't available. **`SettingsControls` decides how a control looks, not where settings live**: `PlaybackEffects` takes its `Toggle` into the player bar's popover and draws its own rows, because `Group` and `SettingRow` are sized for the settings column.
