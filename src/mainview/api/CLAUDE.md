...
- `StorageService` is the one service that fetches when a component mounts rather than off a session change: whether this copy is an installed one, and so has an install to remove, is asked only where the answer is shown. It survives a logout for the same reason — this is a fact about the computer, not about the library.
