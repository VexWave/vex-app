...
- `api/` — `Session`/`Library`/`Artist`/`Playlist`/`Upload`/`Import`/`Binary`/`TrackCache`/`Navigation` services. All are module-level singletons exposed to React via `useSyncExternalStore` (one hook each in `hooks/`), same pattern as the player core. Add new state here, not in component-local state. `presenceBridge.ts` sits here too but is not one of them — no rendered state, no hook, just a player subscription that forwards to bun.
