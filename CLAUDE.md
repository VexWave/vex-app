...
- **The now-playing ring's width and gap are rounded to whole device pixels** (`--dpr`, published by `lib/devicePixelRatio.ts`), not written as plain `2px`. Its band is the padding of a masked box, and padding is not snapped the way a border width is — every edge of the ring's box and of the mask's content box is rounded to the device grid on its own, with no reference to the artwork they wrap, so at fractional display scales the band came out a device pixel thicker on one side than the other. Whole device pixels make the ring an exact integer offset from the artwork on every side, which survives that rounding intact. Both values have to be whole individually, not merely their sum.
