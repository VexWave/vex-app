...
- **Losing the playing track from its collection stops playback** (`PlayerController.syncCollection`) — removed from the playlist, unlinked from the artist, deleted server-side. The queue is what the transport addresses, so a track playing from outside it can't be paused or followed, and an emptied collection disables the transport entirely. It lands in the same state as a queue that ran off its end: nothing loaded, the rest still queued, play starts it over.
